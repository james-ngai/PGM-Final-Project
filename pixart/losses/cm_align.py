...
class ECALoss:
    ...
    def __call__(self, stu_net, tch_net, vae, reward_model, batch, vae_decode_fn=None, rm_preprocess_fn=None):
        ...
        # Data-free via teacher
        # total_steps = 20
        # x = noisy_x
        # stride = len(tch_net.u) // total_steps
        # step = torch.tensor([0]).repeat(B, 1, 1, 1).to(device)
        ...
        #         eps  = tch_net(x, prompt_emb, t, mask=masks, return_eps=True)
        #         x    = x + (r - t) * eps
        #         assert (t - r).min() > 0
        #         step = next_step
        # D_yn = x
        
        # Data-free via student
        x = noisy_x
        mid_t = 4.0
        mid_t = torch.tensor([mid_t]).repeat(B, 1, 1, 1).to(device)

        # TODO: Debug grad checkpointing here.
        # stu_net.eval()
        # with torch.no_grad():
        #     x = stu_net(x, prompt_emb, sigma_init, mask=masks)
        #     x = x + mid_t * torch.randn_like(x)
        #     x = stu_net(x, prompt_emb, mid_t, mask=masks)
        # stu_net.train()
        ...
        r_idx = t_idx + total_steps * stride
