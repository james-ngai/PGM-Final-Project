...
class PixArt(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        for block in self.blocks:
            ...
            if self.training:
                x = checkpoint.checkpoint(module_forward(block), x, y, t0, mask, use_reentrant=False)
            else:
                x = block(x, y, t0, mask)
    ...
    def forward_with_cfg(self, x, timestep, y, cfg_scale, mask=None, **kwargs):
        ...
        B = x.size(0)
        # double input
        combined = torch.concat([x, x], dim=0)
        timestep = torch.concat([timestep, timestep], dim=0)
        mask = torch.concat([mask, mask], dim=0)
        null_y = self.y_embedder.y_embedding[None][:, None].repeat(B, 1, 1, 1) # torch.Size([B, 120, 4096])
        null_y = null_y.to(y.device, y.dtype)
        y = torch.concat([y, null_y], dim=0)
        
        model_out = self.forward(combined, timestep, y, mask) # [:, :4]
        cond_eps, uncond_eps = torch.split(model_out, B, dim=0)
