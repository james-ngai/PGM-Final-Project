...
class PixArtMS(PixArt):
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=1152, depth=28, num_heads=16, mlp_ratio=4.0, class_dropout_prob=0.1, learn_sigma=True, pred_sigma=True, drop_path: float = 0., window_size=0, window_block_indexes=None, use_rel_pos=False, caption_channels=4096, lewei_scale=2., config=None, model_max_length=120, **kwargs):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        if data_info is None:
            # print("using default data_info")
            hw = torch.tensor([[1024, 1024]], dtype=torch.float, device=x.device).repeat(bs, 1)
            ar = torch.tensor([[1.]], device=x.device).repeat(bs, 1)
            data_info = {
                "img_hw": hw,
                "aspect_ratio": ar
            }
    ...
    def forward_with_cfg(self, x, timestep, y, cfg_scale, data_info=None, **kwargs):
        """
